# Šablonų kūrimas ir pritaikymas. Instagram karuselių dizainas ir struktūra

Tikslas: suprasti, kaip kurti ir pritaikyti dizaino šablonus skirtingiems komunikacijos kanalams, išlaikant prekės ženklo vientisumą. Gebėti sukurti strategiškai suplanuotas ir vizualiai nuoseklias Instagram karuseles, atsižvelgiant į informacijos hierarchiją ir dėmesio išlaikymo principus.

## Nuo Brand Kit prie šablonų kūrimo

Praėjusioje paskaitoje mes kalbėjome apie vizualinį identitetą: kas jį sudaro, kodėl jis svarbus, ir sukūrėme savo Canva Brand Kit.

Dabar pereiname prie praktinio Brand Kit panaudojimo, nes tik tada, kai identitetas pradedamas taikyti realiuose dizainuose, jis įgauna prasmę.

Kitaip tariant, Brand Kit yra kaip jūsų garderobas, o šiandien jūs iš jo kuriate aprangą: turite spalvas, šriftus, logotipą, bet dabar reikia mokėti juos derinti taip, kad dizainas atrodytų nuosekliai ir profesionaliai.

## Kas yra dizaino šablonas?

Šablonas (template) – tai dizaino struktūra, kurią galite naudoti daug kartų, keisdami tik turinį.

Kam jis reikalingas:
- sutaupo laiko;
- užtikrina vientisumą;
- leidžia komandai dirbti vienodai;
- padeda išlaikyti profesionalų įvaizdį net keičiant temas.

Pavyzdys:
Jei jūs turite edukacinius postus ir įkvepiančias citatas, kiekvienai kategorijai galima susikurti po šabloną, kuris turi tą pačią struktūrą, bet skirtingą turinį.

![sablono pritaikymo pavyzdys ](https://raw.githubusercontent.com/IamJuniorMB/images/refs/heads/main/social-long-material/12_paskaita/sablono%20panaudojimo%20pavyzdys.avif)

## Canva įrankiai, kurie palengvina šablonų kūrimą

**Grid view** – leidžia matyti visas karuselės skaidres iš viršaus ir jas pertvarkyti.

![Grid view](https://raw.githubusercontent.com/IamJuniorMB/images/refs/heads/main/social-long-material/12_paskaita/Grid%20view.avif)

**Duplicate Page** – greitas būdas išlaikyti struktūrą tarp skaidrių.

![Duplicate Page](https://raw.githubusercontent.com/IamJuniorMB/images/refs/heads/main/social-long-material/12_paskaita/Duplicate%20Page.avif)

**Align ir Position** – centruokite ir lygiuokite tekstus bei elementus vienodai.

![Align ir Position](https://raw.githubusercontent.com/IamJuniorMB/images/refs/heads/main/social-long-material/12_paskaita/Align%20ir%20Position.avif)

**Apply Brand Colors / Fonts (Styles panel)** – vienu paspaudimu pritaiko visą jūsų Brand Kit.

![Apply Brand Colors / Fonts (Styles panel)](https://raw.githubusercontent.com/IamJuniorMB/images/refs/heads/main/social-long-material/12_paskaita/Apply%20Brand%20Colors%20%20Fonts%20%28Styles%20panel%29.avif)

**Grids ir Frames** – tinka nuotraukoms ir vizualinei tvarkai.

![Grids ir Frames](https://raw.githubusercontent.com/IamJuniorMB/images/refs/heads/main/social-long-material/12_paskaita/Grids%20ir%20Frames.avif)

### Template link (šablono nuoroda)

Ši funkcija Canvoje ypač naudinga tada, kai norime pasidalyti savo dizainu su kitais, bet nenorime, kad jie redaguotų originalą.

Kai sukuriate Template link, Canva sugeneruoja specialią nuorodą, o ją paspaudęs žmogus gauna savo kopiją. Tai reiškia, kad kiekvienas gali dirbti su savo versija, o jūsų originalas lieka nepakitęs.

**Kada verta naudoti Template link?**
- Kai norite pasidalyti šablonu su komanda, kad visi kurtų vienodu stiliumi, bet kiekvienas turėtų savo dokumentą.
- Kai kuriate šablonus klientui, kad jis galėtų keisti tekstus, bet neišdarkytų dizaino.
- Kai vedate mokymus, pavyzdžiui, šiandienos praktinėje dalyje galėčiau jums duoti paruoštą Canva failą, o jūs jį atsidarytumėte per  Template link ir dirbtumėte su savo kopija.

#### Kaip tai veikia:
1.  Paspauskite Share.    
2.  Eikite į More ways to publish.    
3.  Pasirinkite Template link.    
4.  Nukopijuokite sugeneruotą nuorodą ir pasidalinkite su kitais.    

![Template link](https://raw.githubusercontent.com/IamJuniorMB/images/refs/heads/main/social-long-material/12_paskaita/template%20link.avif)

Kai kitas žmogus ją paspaus, Canva sukurs naują failą su užrašu „A copy of…“, todėl kiekvienas turės savo kopiją.

**Kodėl tai svarbu?**
- Taip apsaugomas jūsų originalus dizainas, niekas jo nesugadins.
- Tai padeda išlaikyti vienodą stilių komandoje ar grupėje.
- Galite dalintis šablonais mokymams ar projektams, kad visi pradėtų nuo to paties pagrindo.

## Šablonų bazė Canva programoje

https://www.canva.com/templates

Canva šablonų bazė – tai plati paruoštų dizainų biblioteka, leidžianti greitai kurti profesionaliai atrodančius dizainus.

Ji naudinga, nes taupo laiką ir suteikia struktūrą turiniui.

Reikėtų žinoti, kad dalis šablonų yra nemokami, o kai kurie – mokami (Pro), taip pat svarbu patikrinti dokumento formatą (pvz., A4, Instagram Story ir pan.) ir pritaikyti šablonui savo prekės ženklo spalvas, šriftus ir turinį, kad rezultatas būtų autentiškas.

## Informacijos hierarchija

Vienas dažniausių dizaino iššūkių: viskas atrodo svarbu. Kad to išvengtumėte, vadovaukitės informacijos hierarchijos principu:
- Antraštės (Heading) – didžiausios, ryškiausios, kontrastingos.
- Tekstas (Body) – mažesnis, skaitymui patogus, tamsesnis ant šviesaus fono.
- Akcentai (Highlight) – viena spalva ar forma, kuri išsiskiria.

## Instagram karuselių kūrimas

### Kas yra Instagram karuselė?

Instagram karuselė – tai įrašas, kuriame galima įkelti iki 20 skaidrių (nuotraukų ar maketų ar vaizdo įrašų).

Tai tarsi mini prezentacija ar istorija, kurią žmogus slenka iš kairės į dešinę. Šis formatas yra vienas veiksmingiausių būdų išlaikyti dėmesį ir pateikti daugiau vertės viename įraše.

**Dydis: 1080×1350 px (4:5) Instagram įrašui.**

![Instagram carousel template](https://raw.githubusercontent.com/IamJuniorMB/images/refs/heads/main/social-long-material/12_paskaita/Instagram%20carousel%20template.avif)

### Kodėl karuselės taip gerai veikia?

- Žmogus praleidžia daugiau laiko prie įrašo, todėl tai signalas algoritmui, kad turinys įdomus.
- Informacija pateikiama aiškiai ir dozuotai. Viena mintis vienoje skaidrėje, tai padeda išlaikyti dėmesį.
- Puikus būdas kurti pasakojimą (storytelling). Galima vesti žmogų per kelis etapus – nuo problemos iki sprendimo ar nuo idėjos iki veiksmo.

### Instagram karuselių struktūros

**1. Kabliukas → Problema → Sprendimas**
Struktūra pradedama stipriu dėmesį pritraukiančiu teiginiu arba klausimu, kuris įveda į problemą. Toliau pateikiamas sprendimas ir aiškus kvietimas veikti.

Struktūra:
- Kabliukas (klausimas, pažadas, problema)
- Problemos įvardijimas
- Sprendimas ar metodas
- Rezultatas arba nauda
- CTA (išsaugoti, pakomentuoti, sužinoti daugiau)

Naudojimas: pardaviminis ir edukacinis turinys, prekių, paslaugų ar kursų pristatymas.

**2. Klaida → Kodėl tai klaida → Kaip daryti teisingai**
Ši struktūra padeda parodyti kompetenciją ir ugdyti auditoriją per dažniausiai pasitaikančias klaidas.

Struktūra:
- Dažna klaida
- Kodėl ji trukdo rezultatui
- Teisingas sprendimas
- Papildomas patarimas
- CTA

Naudojimas: mokomasis turinys, ekspertinio įvaizdžio formavimas.

**3. Prieš → Po → Kaip tai buvo pasiekta**
Vizualiai ir emociškai stipri struktūra, paremta pokyčio demonstravimu.

Struktūra:
- Pradinė situacija
- Galutinis rezultatas
- Veiksmai ar sprendimai
- Esminė pamoka
- CTA

Naudojimas: klientų istorijos, atvejai, asmeninis prekės ženklas.

**4. Sąrašo struktūra (X patarimų / žingsnių)**
Aiški, lengvai skaitoma struktūra.

Struktūra:
- Pavadinimas ir pažadas
- Vienas patarimas – viena skaidrė
- Baigiamoji skaidrė su CTA

Naudojimas: praktiniai patarimai, „checklist“ tipo turinys.

**5. Mitas → Tiesa **
Padeda keisti auditorijos mąstymą ir formuoti pasitikėjimą.

Struktūra:
- Mitas
- Kodėl jis paplitęs
- Tikroji situacija
- Paaiškinimas
- CTA

Naudojimas: nuomonės formavimas, edukacija.

**6. Žingsniai (Step by step)**
Struktūra orientuota į procesą ir praktinį pritaikymą.

Struktūra:
- Ką auditorija išmoks?
- 1 žingsnis
- 2 žingsnis
- 3 žingsnis
- CTA

Naudojimas: instrukcijos, mokymai, techniniai paaiškinimai.

**7. Klausimas → Atsakymai**
Skatina dialogą ir auditorijos įsitraukimą.

Struktūra:
- Pagrindinis klausimas
- Atsakymai per kelias skaidres
- CTA (pakomentuoti, pasidalinti nuomone)

Naudojimas: bendruomenės stiprinimas, diskusijos, ekspertiškumo demonstravimas.

**8. Istorijos pasakojimo (Storytelling) struktūra**
Emociškai įtraukianti struktūra, padedanti kurti ryšį.

Struktūra:
- Situacijos pradžia
- Problema ar iššūkis
- Lūžio taškas
- Rezultatas
- Pamoka ir CTA

Naudojimas: asmeninis prekės ženklas, patirtys, vertybinis turinys.

**9. Palyginimo struktūra (A vs B)**
Padeda auditorijai lengviau priimti sprendimus.

Struktūra:
- Tema
- Variantų A ypatybės
- Variantų B ypatybės
- Kam kuris tinka?
- CTA

Naudojimas: produktų, planų, paslaugų palyginimas.

**10. Pardaviminė karuselė (subtilus pardavimas)**
Orientuota į problemos sprendimą, o ne tiesioginį pardavimą.

Struktūra:
- Problema
- Pasekmė
- Sprendimas
- Nauda vartotojui
- CTA

Naudojimas: kursai, narystės, paslaugos, produktai.

### Bendri karuselės kūrimo principai

- Viena mintis = viena skaidrė. Neperkraukite informacijos. Kiekviena skaidrė turi turėti aiškų tikslą.  
- Pirmoji ir antroji skaidrė turi užkabinti. Ji turi sustabdyti skrolinimą.  
- Paskutinė skaidrė: raginimas veikti (CTA). Net jei tai tik „Išsaugok šį įrašą“, ji turi būti aiški ir vizualiai išskirta.  
- Išlaikykite vientisumą. Naudokite savo Brand Kit šriftus, spalvas ir elementus.  
  - Change all fonts.  
- Ištestuokite skaitomumą. Peržiūrėkite karuselę telefone. Kas atrodo gerai kompiuteryje, telefone gali būti per maža arba per intensyvu.  
- Svarbu dėmesį skirti ne tik pirmai, bet ir antrai skaidrei, nes dažnai Instagram algoritmas įrašo rodymą naujienų sraute pradeda būtent nuo antros skaidrės.

### Papildomas patarimas iš praktikos

Karuselės yra ne tik dizainas, bet ir turinio strategijos įrankis. Kurdami jas, mąstykite ne „ką noriu parodyti“, o „ką žmogus išsineš“.  

**Klausimas, kuris visada padeda:** „Jei žmogus pamatytų tik šitą karuselę, ar jis suprastų, ką noriu pasakyti?“  


### Prieš publikuojant visada patikrink!

- Ar pirma ir antra skaidrė sustabdo skrolinimą?  
- Ar viena mintis vienoje skaidrėje?  
- Ar hierarchija aiški: antraštė → pagrindinis tekstas → CTA?  
- Ar tekstas įskaitomas telefone?  
- Ar išmatavimai yra 1080×1350?  

Karuselių pavyzdžiai:
1. https://www.instagram.com/p/DNXq8BNMPZC/?igsh=MXQ4Z3R2ZHR6eTI1cQ%3D%3D  
2. https://www.instagram.com/p/DSDQEJdiDXi/?img_index=1 
3. https://www.instagram.com/p/DBLh-pYOucB/?igsh=MWZudGdpZTcyMWR6bg%3D%3D  
4. https://www.instagram.com/p/DIHKXqUIdLY/?igsh=MXhsazY5eTR6YW5oYg%3D%3D  
5. https://www.instagram.com/p/DBLrtGJucQc/?img_index=1  
6.  https://www.instagram.com/p/DNF5CKRtRFH/?img_index=1

# Praktinės užduotys

### 2 užduotis. Karuselės kūrimas Canvoje

**Tikslas:** pritaikyti anksčiau sukurtą Canva Brand Kit praktikoje, išmokti kurti vientisą ir strategiškai suplanuotą Instagram karuselę pagal savo prekės ženklo stilių.

**Užduotis:**
- Atsidarykite Canva, susiraskite tinkamą karuselės šabloną, pakeiskite jame esančia informaciją ir naudodami savo Brand Kit elementus (spalvas, šriftus, logotipą ir stiliaus gaires) sukurkite Instagram karuselę (1080 × 1350 px).
- Pasirinkite vieną iš karuselių struktūrų.
- Karuselė turėtų turėti 5–10 skaidrių, aiškią struktūrą ir raginimą veikti (CTA) paskutinėje skaidrėje.
- Dizainui naudokite tik savo Brand Kit spalvas, šriftus.
- Sukūrus dizainą, nukopijuokite viešą dizaino nuorodą (Share → Anyone with the link → Can Edit).
- Įkelkite karuselės Canva nuorodą į atsiskaitymo formą. 

Vertinama karuselės struktūra, dizaino vientisumas pagal Brand Kit, informacijos aiškumas ir tinkamas nuorodos pateikimas atsiskaitymui.

### Papildoma užduotis. Instagram karuselės analizė

**Tikslas:** išmokti atpažinti, kas daro karuselę veiksmingą arba neveiksmingą: nuo struktūros ir dizaino iki turinio aiškumo bei emocinio poveikio.

**Užduotis:**
- Nueikite į Instagramnir pasirinkite vieną karuselę, ji gali būti gera arba arba nelabai pavykusi, svarbu, kad jums keltų reakciją (patinka arba erzina).  
- Padarykite ekrano nuotraukas arba nukopijuokite įrašo nuorodą.  
- Atsakykite į šiuos klausimus (trumpai, bet argumentuotai):    
  - **A.** Struktūra ir turinys  
    - Kiek skaidrių?     
    - Ar matyti struktūra?  
  - **B**. Dizainas ir vizualai  
    - Kokios spalvos ir šriftai naudojami, ar jie dera tarpusavyje?  
    - Ar pakankamas kontrastas?    
    - Ar išlaikytas vientisumas tarp skaidrių?    
    - Ar tekstas lengvai įskaitomas?  
  - **C.** Dėmesio išlaikymas  
    - Ar pirma skaidrė patraukia dėmesį? Kodėl?    
    - Ar antra skaidrė skatina slinkti toliau?    
    - Ar paskutinėje skaidrėje yra aiškus kvietimas veikti (CTA)?  
  - **D.** Įžvalga  
    - Kas šioje karuselėje veikia / neveikia ir kodėl?    
    - Ką iš šio pavyzdžio pasiimtum (ar vengsi) kurdamas savo dizainą?  
- Atsakymus pateikite Canvoje ar PDF formatu.  

Vertinama, ar studentas geba pagrįstai įvertinti karuselės struktūrą, dizaino sprendimus ir komunikacijos aiškumą, remdamasis konkrečiais pavyzdžiais.  
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTkzNTY1NDQ3LDIwNDAwMzM3NjUsLTk2MD
g2NTc2NiwxNzA2Mzk0Mjg0LC0yMDY5NzE4MjU5LC03MzczNDY5
NjZdfQ==
-->